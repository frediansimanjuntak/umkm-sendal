                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                        <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                            ...
                            <div>
                                <a href="{{route('products.create')}}" class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800">Add Product</a>
                            </div>
                        </div>
                                ...
                                @else
                                    @foreach ($products as $product)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td class="px-6 py-4">
                                                <a href="{{route('product.product-images.index', $product)}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Images</a>
                                                ...
                                                <x-modal name="delete-confirmation-{{$product->id}}" :shows="$errors->productDeletion->isNotEmpty()" focusable>
                                                    <form method="POST" action="{{route('products.destroy', $product)}}" class="p-6">
                                                        @csrf
                                                        @method('DELETE')
                                                        <h2>Want to delete? {{$product->name}}</h2>
